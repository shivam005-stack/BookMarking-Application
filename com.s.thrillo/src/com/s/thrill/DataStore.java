...
public class DataStore {
	...
	private static List<com.s.thrillo.entities.UserBookmark> userBookmarks = new ArrayList<>();
		...
		ResultSet rs= stmt.executeQuery(query);
		  ...
		  while(rs.next()) {
			  ...
			  UserType userType=UserType.values()[user_type_id];
			    ...
			    User user= UserManager.getInstance().createUser(user_type_id, email, password, "", "", gender, userType/*, values[7]*/);
			    users.add(user);
		  }
	}
	...
	private static void loadBooks(Statement stmt) throws SQLException {		    	
    	...
    	List<com.s.thrillo.entities.Bookmark> bookmarkList = new ArrayList<>();
    	while (rs.next()) {
    		long id = rs.getLong("id");
			String title = rs.getString("title");
			...
			com.s.thrillo.constants.BookGenre genre = com.s.thrillo.constants.BookGenre.values()[genre_id];
    		...
    		com.s.thrillo.entities.Bookmark bookmark = com.s.thrillo.managers.BookmarkManager.getInstance().createBook(id, title, publicationYear, publisher, authors, genre, amazonRating/*, values[7]*/);
    		bookmarkList.add(bookmark); 
    	}
    	bookmarks.add(bookmarkList);
    }	
	
	public static void add(com.s.thrillo.entities.UserBookmark userBookmark) {
